...
static qboolean S_WAV_CodecOpenStream(snd_stream_t* stream) {
    ...
    // the FS_*() functions: We will manipulate the file by
    // ourselves from now on.
    if (!WAV_ReadRIFFHeader(stream->name, stream->fh.file, &stream->info)) {
        return false;
    }

    // Reset to data position.
    stream->fh.start = ftell(stream->fh.file);
...
snd_codec_t wav_codec = {
    ...
    .ext = "wav",
    .initialize = S_WAV_CodecInitialize,
    .shutdown = S_WAV_CodecShutdown,
    .codec_open = S_WAV_CodecOpenStream,
    .codec_read = S_WAV_CodecReadStream,
