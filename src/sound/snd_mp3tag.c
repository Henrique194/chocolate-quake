...
static qboolean MP3_IsId3v2(const unsigned char* data, size_t length) {
    ...
    // bytes 6-9 are the ID3v2 tag size: a 32 bit 'synchsafe' integer,
    // i.e. the highest bit 7 in each byte zeroed.
    // i.e.: 7 bit information in each byte -> effectively a 28 bit value.
...
static qboolean Mp3_IsMusicMatch(const unsigned char* data, long length) {
    ...
    if (!IS_DIGIT(data[32]) || data[33] != '.' || !IS_DIGIT(data[34]) || !IS_DIGIT(data[35])) {
        return false;
    }
    ...
    if (stream->fh.length >= 128) {
        ...
        if (MP3_IsId3v1(buf, 128)) {
            if (!atend) {
                ...
                if (Mp3_IsMusicMatch(buf + 128 - 48, 48)
                    || MP3_IsApeTag(buf + 128 - 32, 32)
                    || MP3_IsLyrics3Tag(buf + 128 - 15, 15)) {
                    return 0;
                }
            }
...
static int MP3_ProbeMMTag(snd_stream_t* stream, unsigned char* buf) {
    ...
    if (stream->fh.length >= 68) {
        ...
        if (Mp3_IsMusicMatch(buf, 48)) {
            len = MP3_GetMusicMatchLen(stream);
            if (len < 0) {
                return -1;
            }
...
static int MP3_ProbeLyrics3(snd_stream_t* stream, unsigned char* buf) {
    ...
    if (stream->fh.length >= 15) {
        ...
        if (len == 2) {
            ...
            len = MP3_GetLyrics3v1Len(stream);
            if (len < 0) {
                return -1;
            }
            stream->fh.length -= len;
...
int MP3_SkipTags(snd_stream_t* stream) {
    ...
    } else if (MP3_IsApeTag(buf, readsize)) {
        // APE tag might be at the start (discouraged but
        // not forbidden, either). Read the header.
        len = MP3_GetApeLen(buf);
        if (len >= stream->fh.length) {
            goto fail;
        }
    ...
    if ((c_mm = MP3_ProbeMMTag(stream, buf)) < 0) {
        goto fail;
    }
    // ID3v1 tag is at the end
    if (MP3_ProbeId3v1(stream, buf, !c_mm) < 0) {
        goto fail;
    }
    ...
    for (;;) {
        if (!c_lyr) {
            ...
            if ((c_lyr = MP3_ProbeLyrics3(stream, buf)) < 0) {
                goto fail;
            }
            if (c_lyr) {
                continue;
        ...
        if (!c_ape) {
            if ((c_ape = MP3_ProbeApeTag(stream, buf)) < 0) {
                goto fail;
            }
            if (c_ape) {
                continue;
